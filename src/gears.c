...
void gears_draw(void)
{
    angle += 2.0;
    glPushMatrix();
    glRotatef(view_rotx + angle * 2.0, 1.0, 0.0, 0.0);
    glRotatef(view_roty, 0.0, 1.0, 0.0);
    glRotatef(view_rotz - angle * 0.5, 0.0, 0.0, 1.0 );
...
void gears_init_scene(void)
{
    ...
    glMaterialfv(GL_FRONT, GL_SPECULAR, white);
    glColor3fv(green);
    gear(0.5, 2.0, 1.0, 10, 0.7); /* The small gear with the smaller hole, to the right. */
    glEndList();
    ...
    glMaterialfv(GL_FRONT, GL_SPECULAR, white);
    glColor3fv(blue);
    gear(1.3, 2.0, 1.0, 10, 0.7); /* The small gear above with the large hole. */
    glEndList();
}
