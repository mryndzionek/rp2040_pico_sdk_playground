...
static void compute_amps(uint32_t data[FFTSIZE])
{
    ...
    for (size_t j = 0; j < NUM_PIXELS / 2; j++)
    {
        ...
        {
            fft_amps[j] = a;
            fft_amps[NUM_PIXELS - 1 - j] = a;
        }
        else
        {
            fft_amps[j] -= 1.0 / (1.0 * (SAMPLERATE / FFTSIZE));
            fft_amps[NUM_PIXELS - 1 - j] -= 1.0 / (1.0 * (SAMPLERATE / FFTSIZE));
        }
    }
}
